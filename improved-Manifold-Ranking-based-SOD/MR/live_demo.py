...
def step_detct(img):
    ...
    sal = mr_sal.saliency(src)

    h,w = src.shape[0],src.shape[1]

    sal = cv2.resize(sal, (w, h)).astype(sp.uint8)
    sal = cv2.normalize(sal, None, 0, 255, cv2.NORM_MINMAX)

    outsal = cv2.applyColorMap(sal,colormap=cv2.COLORMAP_BONE)  # 可以改变颜色变化 Bone\JET、PINK   可PARULA、WINTER、SPRING、hot

    # outsal = cv2.cvtColor(outsal, cv2.COLOR_HSV2BGR)
    # cv2.imshow('ret', outsal)
    # cv2.waitKey(0)
    return outsal
